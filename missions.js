...
const MISSIONS_CONFIG = {
    ...
    "day_2_kid14_protocol": { tag: "versus", day: 2, title: "Protocolo Shōgun", role: "kid14", xp: 20, location: "Metro / Tren", render: () => `<p class="mission-desc">Guía a la familia al hotel.</p><button id="btn" class="btn-primary" style="width:100%">Llegamos</button>`, attachEvents: (role) => { attachCameraFlow('btn', 'day_2_kid14_protocol', role || currentUser); } },
    "day_3_kid14_architect": { tag: "writing", day: 3, title: "Arquitecto del Castillo", correctAnswer: "~70m a 90m (Foso del castillo)",  correctAnswer: "~70m a 90m (Foso del castillo)", role: "kid14", xp: 20, location: "Castillo Osaka", render: () => `<p class="mission-desc">Analiza la defensa del Shogun. Abre Google Maps en modo satélite y utiliza la herramienta de medida para calcular la distancia real aproximada (en metros) del foso de agua que protege el Castillo de Osaka.</p><input type="number" id="d" placeholder="Metros calculados..."><button id="btn" class="btn-primary">Enviar Informe de Asedio</button>`, attachEvents: () => { document.getElementById('btn').addEventListener('click', () => submitMission('day_3_kid14_architect', {type:'number', data:document.getElementById('d').value})); } },
    ...
    "day_4_fam_yakiniku": { tag: "versus", day: 4, title: "Maestro Yakiniku", role: "both", xp: 20, location: "Restaurante", render: () => `<p class="mission-desc">Trabajo en equipo en la barbacoa.</p><button id="btn" class="btn-primary">Completado</button>`, attachEvents: (role) => { attachCameraFlow('btn', 'day_4_fam_yakiniku', role, true); } },
    ...
    "day_5_kid14_kanji": { tag: "writing", day: 5, title: "Caligrafía Zen", role: "kid14", xp: 15, location: "Nara", render: () => `<p class="mission-desc">La caligrafía Shodō es un arte milenario. Usando tu dedo como si fuera un pincel de bambú, dibuja el Kanji de "Persona" (人) o "Montaña" (山) en este lienzo digital.</p><canvas id="c" style="border:1px solid #ccc; height:200px; width:100%;"></canvas><button id="btn" class="btn-primary">Enviar Caligrafía</button>`, attachEvents: (role) => { attachCameraFlow('btn', 'day_5_kid14_kanji', role || currentUser); } },
    ...
    "day_5_kid14_eng": { tag: "writing", day: 5, title: "Ingeniero Todai-ji", correctAnswer: "El área es inmensa, cualquier cálculo de ~2500m2 a 3000m2 es válido.",  correctAnswer: "El área es inmensa, cualquier cálculo de ~2500m2 a 3000m2 es válido.", role: "kid14", xp: 20, location: "Todai-ji", render: () => `<p class="mission-desc">Cálculo área base.</p><input type="text" id="a"><button id="btn" class="btn-primary">Enviar</button>`, attachEvents: () => { document.getElementById('btn').addEventListener('click', () => submitMission('day_5_kid14_eng', {type:'text', data:document.getElementById('a').value})); } },
        "day_5_kid9_buda": {
    ...
    "day_7_kid14_guia": { tag: "video", day: 7, title: "Guía en 60s", role: "kid14", xp: 20, location: "Gion", render: () => `<div id="timer">60</div><input type="file" accept="video/*" capture="environment" id="v"><button id="b" class="btn-primary">Enviar</button>`, attachEvents: (role) => { attachCameraFlow('b', 'day_7_kid14_guia', currentUser, false); }},1000); document.getElementById('b').addEventListener('click', () => submitMission('day_7_kid14_guia', {type:'text', data:'Guía'})); } },
    "day_7_kid9_tea": { tag: "sensors", day: 7, title: "Té Shogun", role: "kid9", xp: 15, location: "Sannenzaka", render: () => `<p class="mission-desc">Imagínate que llevas una bandeja con té hirviendo para el Shogun. Debes caminar 20 metros manteniendo el móvil totalmente plano (como una bandeja). Si se inclina demasiado, ¡el té se derrama!</p><button id="btn" class="btn-primary">Iniciar Servicio</button>`, attachEvents: (role) => { attachCameraFlow('btn', 'day_7_kid9_tea', role || currentUser); } },
    ...
    "day_7_fam_otowa": { tag: "versus", day: 7, title: "Reto Agua Sagrada", role: "both", xp: 30, location: "Kiyomizu", render: () => `<p>Elegid un don en secreto.</p><select id="s9"><option value="estudios">Estudios</option><option value="amor">Amor</option><option value="salud">Salud</option></select><select id="s14"><option value="estudios">Estudios</option><option value="amor">Amor</option><option value="salud">Salud</option></select><button id="b" class="btn-primary">Enviar</button>`, attachEvents: (role) => { attachCameraFlow('b', 'day_7_fam_otowa', role || currentUser, true); } },
    "day_7_kid9_otowa": { tag: "versus", day: 7, title: "Chorros Destino", role: "kid9", xp: 15, location: "Kiyomizu", render: () => `<p class="mission-desc">Elige SOLO UN chorro.</p><button id="btn" class="btn-primary">Beber</button>`, attachEvents: (role) => { attachCameraFlow('btn', 'day_7_kid9_otowa', role || currentUser); } },
    "day_7_kid14_madera": { tag: "writing", day: 7, title: "Magia Madera", correctAnswer: "Se ensambla con un sistema de encaje de madera (Kigumi) que resiste sismos.",  correctAnswer: "Se ensambla con un sistema de encaje de madera (Kigumi) que resiste sismos.", role: "kid14", xp: 15, location: "Kiyomizu", render: () => `<p class="mission-desc">Explica cómo aguanta sin clavos.</p><textarea id="t"></textarea><button id="btn" class="btn-primary">Enviar</button>`, attachEvents: () => { document.getElementById('btn').addEventListener('click', () => submitMission('day_7_kid14_madera', {type:'text', data:document.getElementById('t').value})); } },
    ...
    "day_8_kid14_bamboo_eng": { tag: "writing", day: 8, title: "Ingeniero Bosque", correctAnswer: "La altura varía entre 15m y 30m.",  correctAnswer: "La altura varía entre 15m y 30m.", role: "kid14", xp: 15, location: "Arashiyama", render: () => `<input type="number" id="n"><button id="btn" class="btn-primary">Enviar</button>`, attachEvents: () => { document.getElementById('btn').addEventListener('click', () => submitMission('day_8_kid14_bamboo_eng', {type:'number', data:document.getElementById('n').value})); } },
    "day_8_kid14_codigo": { tag: "writing", day: 8, title: "El Código del Jardín", role: "kid14", xp: 20, location: "Tenryu-ji", render: () => `<input id="e1"><input id="e2"><input id="e3"><button id="b" class="btn-primary">Enviar</button>`, attachEvents: (role) => { attachCameraFlow('b', 'day_8_kid14_codigo', role || currentUser); } },
    "day_8_kid9_rake": { tag: "writing", day: 8, title: "Rastrillo Zen", role: "kid9", xp: 15, location: "Tenryu-ji", render: () => `<canvas id="rake" style="height:150px; width:100%; border:1px solid #ccc;"></canvas><button id="btn" class="btn-primary">Enviar</button>`, attachEvents: (role) => { attachCameraFlow('btn', 'day_8_kid9_rake', role || currentUser); } },
    ...
    "day_9_kid14_torii_count": { tag: "writing", day: 9, title: "Código Torii", correctAnswer: "Es imposible contarlos todos (hay más de 10,000), premia el esfuerzo.",  correctAnswer: "Es imposible contarlos todos (hay más de 10,000), premia el esfuerzo.", role: "kid14", xp: 15, location: "Fushimi", render: () => `<input type="number" id="n"><button id="btn" class="btn-primary">Enviar</button>`, attachEvents: () => { document.getElementById('btn').addEventListener('click', () => submitMission('day_9_kid14_torii_count', {type:'number', data:document.getElementById('n').value})); } },
    ...
    "day_10_kid9_nishiki": { tag: "economy", day: 10, title: "Maestro Chatarra", role: "kid9", xp: 15, location: "Nishiki", render: () => `<button id="btn" class="btn-primary">Pagado</button>`, attachEvents: (role) => { attachCameraFlow('btn', 'day_10_kid9_nishiki', role || currentUser); } },
