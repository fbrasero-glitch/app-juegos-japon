...
# Función para encontrar y extraer un bloque de objeto JS por su clave
def get_object_range(key, text):
    start_key = f'"{key}":'
    ...
    # El bloque termina en idx. Verificar si le sigue },
    end_idx = idx
    if text[end_idx:end_idx+2] == '},':
        end_idx += 2
    elif text[end_idx:end_idx+1] == '}':
        end_idx += 1
        
    return (start_idx, end_idx)

# Corregir los bloques rotos (si los hay)
# El error de Node indica que hay un "day_3_kid9_foso" pegado dentro de "day_6_kid9_ninja_steps"
# Vamos a limpiar MISSIONS_CONFIG primero

# Reemplazo de misiones con seguridad
PHYSICAL_MISSIONS = {
    "day_3_kid9_foso": """{
...
# 1. Extraer MISSIONS_CONFIG completo y reconstruirlo limpiamente
start_marker = 'const MISSIONS_CONFIG = {'
end_marker = '};'
...
# Parsear misiones existentes (muy básico)
# Extraemos cada par "id": { ... }
missions_found = {}
# Usamos un buscador de llaves balanceadas
search_idx = config_text.find('{') + 1
while search_idx < len(config_text) - 2:
    key_match = re.search(r'"(day_.*?|any_.*?)":', config_text[search_idx:])
    if not key_match: break
    
    m_id = key_match.group(1)
    m_start = search_idx + key_match.end()
    
    # Encontrar el { correspondiente
    brace_start = config_text.find('{', m_start)
    count = 1
    idx = brace_start + 1
    while count > 0 and idx < len(config_text):
        if config_text[idx] == '{': count += 1
        elif config_text[idx] == '}': count -= 1
        idx += 1
    
    m_body = config_text[brace_start:idx]
    missions_found[m_id] = m_body
    search_idx = idx

# 2. Actualizar con las misiones físicas
for mId, mBody in PHYSICAL_MISSIONS.items():
    missions_found[mId] = mBody

# 3. Reconstruir MISSIONS_CONFIG ordenado por día
sorted_ids = sorted(missions_found.keys(), key=lambda x: (int(re.search(r'day_(\d+)', x).group(1)) if 'day_' in x else 99, x))
new_config = "const MISSIONS_CONFIG = {\n"
for mId in sorted_ids:
    new_config += f'    "{mId}": {missions_found[mId]},\n'
new_config = new_config.rstrip(',\n') + "\n};"

# 4. Reemplazar en el contenido total
new_content = content[:start_idx] + new_config + content[end_idx:]

with open('app.js', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("MISSIONS_CONFIG reconstruido y limpio con todas las misiones físicas.")
